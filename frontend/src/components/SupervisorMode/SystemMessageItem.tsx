...
import { systemMessageUpdate } from '../../api';
  ...
  itemText
	...
	const handleSaveEdit = async (e: any) => {
		...
		if(editTitle == itemText){
			setErrorRename("Error: Enter a new system message to proceed.")
			setTimeout(() => {
		...
		let response = await systemMessageUpdate(editTitle);
		if(!response.ok){
			setErrorRename("Error: could not update system message")
			setTimeout(() => {
				setTextFieldFocused(true);
				setErrorRename(undefined);
				if (textFieldRef.current) {
			...
			setEdit(false)
			appStateContext?.dispatch({ type: 'UPDATE_SYSTEM_MESSAGE', payload: editTitle})
			setEditTitle("");
		}
	}

	const systemMessageTitleOnChange = (e: any) => {
	...
	return (
		<Stack
			...
			tabIndex={0}
			aria-label='system message item'
		...
		>
			{edit ? <>
				<Stack.Item 
					style={{ width: '100%' }}
				>
					<form aria-label='edit system message form' onSubmit={(e) => handleSaveEdit(e)} style={{padding: '5px 0px'}}>
						<Stack horizontal verticalAlign={'start'}>
							<Stack.Item>
								<TextField
									componentRef={textFieldRef}
									autoFocus={textFieldFocused}
									className={styles.systemMessageInputTextArea}
									placeholder={itemText}
									multiline
									autoAdjustHeight
									borderless
									value={editTitle}
									onChange={systemMessageTitleOnChange}
				...
				<Stack horizontal verticalAlign={'center'} style={{ width: '100%' }}>
					<div className={styles.systemMessageTitle} onClick={onEdit}>{truncatedTitle}</div>
